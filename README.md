...
scripts/
├── check-payload.{sh,ps1}     # 公開前チェック：個人ファイル混入・無視されうる設定キー等を検査
...
```bash
bash scripts/check-payload.sh .                 # 個人ファイル混入・設定キーを機械チェック（CI 可）
bash scripts/clean-test-env.sh /path/to/share   # 自分の設定を切り離し共有資産だけで起動して動作確認
```

PowerShell では `scripts\check-payload.ps1 -Share .` / `scripts\clean-test-env.ps1 -Share <path>`。
`check-payload` は `<Share>` が git リポジトリなら **Git 追跡されているか**で配布可否を判定する
（gitignore 済みの個人ファイルが実在しても WARN 止まりで FAIL にしない）。
