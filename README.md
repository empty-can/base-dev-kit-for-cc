...
| `.cmd` | 使用禁止（`.bat` に一本化） | ― |
| `.reg` / `.ini` | スコープ外 | `permissions.deny` で R/W 禁止 |
...
- [Claude Code](https://claude.ai/code) CLI（`if` 付き hook を使うため **v2.1.85 以上**）
- Node.js v18 以上（MCP サーバー用）
- Python（CPython 3.x）―― `.bat` を扱う hook と `win-file-encoding` skill 用。無くても壊れないが、
  `.bat` は読めなくなる（`permissions.deny` により編集は常に拒否されるので、原本が壊れることはない）
- Git
