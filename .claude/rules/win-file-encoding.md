...
UTF-8 + BOM なので Claude のツールがそのまま扱える。**変換は不要**。ただし Claude Code の
Edit / Write は過去に **CRLF を二重化する不具合（v2.1.89 修正）／CRLF ファイルの改行を黙って変換する不具合
（v2.1.77 修正）**を出しており、**改行と BOM の保全は仕様として保証されていない**（公式 docs にファイル操作
ツールの改行・文字コードの規定は無い）。編集後に壊れていないか確認するなら:
