...
└── reports/                   # セッション成果物（.gitignore 対象）
    └── <activity-name>/       # 活動・ブランチ単位でサブフォルダを切る
        ├── plans/             # 計画書・クロスレビュー結果
        └── <topic>/           # 調査レポート・レビュー成果物等
...
### セッション成果物（reports/）の配置規約

- **原則**: `.claude/reports/<activity-name>/` 配下に活動（≒ブランチ）単位でサブフォルダを作成する
- **計画書・構成プラン**: `<activity-name>/plans/` に配置
- **調査レポート**: `<activity-name>/<topic>/` に配置（例: `design-base-kit/claude-code-research/`）
- **作業ログ・進捗**: `<activity-name>/work-<date>/` に配置
- **`.claude/plans/` は使用しない**（reports/ 配下の `plans/` に統一）
